...
<?php
	...
	// ------------------------------------------------------------------------------------------------------------------------------
	// UPLOAD salesrep - DELETE AND RELOAD
	// ------------------------------------------------------------------------------------------------------------------------------
 
	foreach (glob("MI-DAS_salesrep*.csv") as $file) 
	{
		if (($handle = @fopen($file, "r")) !== FALSE) 
		{
			// Delete all existing salesrep rows
			
			$query = "TRUNCATE salesrep"; 
			...
			while (($data = fgetcsv($handle, 500, ";")) !== FALSE)
			{
				...
				if (!$repcode == "")
				{
					...
					$query = "INSERT INTO salesrep(repcode, name) VALUES('".$repcode."','".$name."') ON DUPLICATE KEY UPDATE name = '$name'";
					$result = mysqli_query($link, $query) or logerror($query,mysqli_error($link));
				}
			}
			fclose($handle);
