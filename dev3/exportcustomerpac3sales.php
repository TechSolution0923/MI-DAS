<?php
	...
	header('Content-Disposition: attachment; filename=customerpac3sales.csv');
	...
	while ($custrow = mysqli_fetch_row($custresult)) 
	{
		...
		$pac3query = "SELECT code FROM pac3";
		$pac3result = mysqli_query($link, $pac3query) or logerror($pac3query,mysqli_error($link));
		while ($pac3row = mysqli_fetch_row($pac3result)) 
		{
			$pac3code = $pac3row[0];
			...
			for ($x = 0; $x <= 35 ; $x++)
			{
				...
				$query = "SELECT $salesfield FROM customerpac3sales WHERE account = '$account' AND pac3code = '$pac3code'";
				...
				// Write to the csv
				fputcsv($f, array($account, $codelevel, $pac3code, $yearmonth, $salesvalue));
			}
		}
	}
