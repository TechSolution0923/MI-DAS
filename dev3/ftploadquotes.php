...
<?php
	...
	// ------------------------------------------------------------------------------------------------------------------------------
	// UPLOAD QUOTES - INSERT/UPDATE
	// ------------------------------------------------------------------------------------------------------------------------------
	
	foreach (glob("MI-DAS_quotes*.csv") as $file) 
	{
		if (($handle = @fopen($file, "r")) !== FALSE) 
		{
			$quotessloaded = "Y"; // Set this flag so that quotes kpis will be recalculated in loadsalesorderkpis.php
			while (($data = fgetcsv($handle, 500, ";")) !== FALSE)
			{
				...
				$account       = preg_replace("/[^a-zA-Z0-9-\/\s]/", "",$data[4]);
				$headerstatus  = $data[5];
