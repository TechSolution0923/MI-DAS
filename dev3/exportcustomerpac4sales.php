<?php
	...
	header('Content-Disposition: attachment; filename=customerpac4sales.csv');
	...
	while ($custrow = mysqli_fetch_row($custresult)) 
	{
		...
		$pac4query = "SELECT code FROM pac4";
		$pac4result = mysqli_query($link, $pac4query) or logerror($pac4query,mysqli_error($link));
		while ($pac4row = mysqli_fetch_row($pac4result)) 
		{
			$pac4code = $pac4row[0];
			...
			for ($x = 0; $x <= 35 ; $x++)
			{
				...
				$query = "SELECT $salesfield FROM customerpac4sales WHERE account = '$account' AND pac4code = '$pac4code'";
				...
				// Write to the csv
				fputcsv($f, array($account, $codelevel, $pac4code, $yearmonth, $salesvalue));
			}
		}
	}
