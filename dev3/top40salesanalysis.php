<?php
	...
	header('Content-Disposition: attachment; filename=top40salesanalysis.csv');
	...
	$salesthisyearcols = "msales1 ";
	$qtythisyearcols = "mquantity1 ";
	
	for ($x = 2; $x < $curmonth; $x++) // curmonth is the current month from the systems table. This should build something like msales0 + msales1 + msales2 etc up to current month
	{				
	...
	$saleslastyearcols = "msales13 ";
	$qtylastyearcols = "mquantity13 ";
	
	for ($x = 14; $x < $curmonth + 12; $x++) 
	{				
	...
	$custquery = "SELECT customer.account, customer.repcode,customersales.$salesthisyearcols, customersales.$saleslastyearcols, customersales.ysales1 FROM customer LEFT JOIN customersales ON customersales.account = customer.account";
	...
	while ($custrow = mysqli_fetch_row($custresult)) 
	{
		...
		$toyearmonth   = $curyear.str_pad(($curmonth - 1),2,'0',STR_PAD_LEFT);
