			...
			<div class="row">
				...
				<?php echo form_open('users/details', array("role"=>"form", "enctype"=>"multipart/form-data", "onsubmit"=>"return checkUnique();"));?>
					<div class="col-xs-8">
						...
						<div class="form-group">
						  <label>company</label>
						   <select class="form-control" name="company" id="company" required <?php echo $disabled;?>>
							<?php foreach($companyes as $option) { 
									$current_company = "";
									if(isset($userDetail['company'])) {
										$current_company = $userDetail['company'];
									}
							?>
							<option value="<?php echo 0==$option['company']?"":$option['company'];?>" <?php if($current_company==$option['company']) { echo "selected";} else { echo "";}?>><?php echo $option['name'];?></option>
							<?php }?>
						   </select>
