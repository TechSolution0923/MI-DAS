...
<?php
	...
	// ------------------------------------------------------------------------------------------------------------------------------
	// UPLOAD PRODUCT DISCOUNT GROUPS - DELETE AND RELOAD
	// ------------------------------------------------------------------------------------------------------------------------------
 
	foreach (glob("MI-DAS_discgroup*.csv") as $file) 
	{
		if (($handle = @fopen($file, "r")) !== FALSE) 
		{
			// Delete all existing PAC3 rows
			
			$query = "TRUNCATE TABLE proddiscgroup"; 
			...
			while (($data = fgetcsv($handle, 500, ";")) !== FALSE)
			{
				...
				if (!$code == "")
				{
					$query = "INSERT INTO proddiscgroup(code, description) VALUES('".$code."','".$desc."')";
					$result = mysqli_query($link, $query) or logerror($query,mysqli_error($link));
