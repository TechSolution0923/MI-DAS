<!-- Only loads the demand from the sales analysis file. File also has a different name. Used for testing -->
...
<?php
	...
	foreach (glob("demandsalesanal*.csv") as $file) 
	{
		//$file = "salesanal.csv";
		if (($handle = @fopen($file, "r")) !== FALSE) 
		{
			...
			while (($data = fgetcsv($handle, 500, ";")) !== FALSE)	
			{
				...
				if (!$branch == "")
				{
					// Set the salesanalflag
					
					$salesanalloaded = "Y";

					// Get the current customer details
					$custquery = "SELECT repcode, currency, accounttype FROM customer WHERE account = '$account'";
					...
					$query = "INSERT INTO salesanalysis(branch, account, repcode, pac1, pac2, pac3, pac4, currepcode, curpac1code, curpac2code, curpac3code, curpac4code, prodcode, quantity, unit, sales, cost, date, orderno, invoiceno, yearmonth,
							 ordtype, magic, exchrate, currency) 
							VALUES($branch,'$account','$custrepcode','$pac1','$pac2','$pac3','$pac4','$custrepcode','$pac1','$pac2','$pac3','$pac4','$prodcode',$quantity,'$unit',$sales,$cost,'$date',$orderno,$invoiceno,$yearmonth,'$ordtype',$magic, $exchangerate, '$currency')";
					...
					$demandmonthquery = "INSERT INTO demand(productcode, branch, year, period, type, forecastdemandqty, demandqty, demandnum, demandval, extdemandqty, extdemandnum, extdemandval, intdemandqty, intdemandnum, intdemandval) VALUES('$prodcode',$branch, $year, $month, 'M', 0, $quantity, 1, $cost, $extdemandqty, $extdemandnum, $extdemandval, $intdemandqty, $intdemandnum, $intdemandval) ON DUPLICATE KEY UPDATE demandqty = demandqty + $quantity, demandnum = demandnum + 1, demandval = demandval + $cost, extdemandqty = extdemandqty + $extdemandqty, extdemandnum = extdemandnum + $extdemandnum, extdemandval = extdemandval + $extdemandval, intdemandqty = intdemandqty + $intdemandqty, intdemandnum = intdemandnum + $intdemandnum, intdemandval = intdemandval + $intdemandval"; 

					$demandmonthresult = mysqli_query($link, $demandmonthquery) or logerror($demandmonthquery,mysqli_error($link));

					//$demandweekquery = "INSERT INTO demand(productcode, branch, year, period, type, forecastdemandqty, demandqty, demandnum, demandval, extdemandqty, extdemandnum, extdemandval, intdemandqty, intdemandnum, intdemandval) VALUES('$prodcode',$branch, $year, $week, 'W', 0, $quantity, 1, $cost, $extdemandqty, $extdemandnum, $extdemandval, $intdemandqty, $intdemandnum, $intdemandval) ON DUPLICATE KEY UPDATE demandqty = demandqty + $quantity, demandnum = demandnum + 1, demandval = demandval + $cost, extdemandqty = extdemandqty + $extdemandqty, extdemandnum = extdemandnum + $extdemandnum, extdemandval = extdemandval + $extdemandval, intdemandqty = intdemandqty + $intdemandqty, intdemandnum = intdemandnum + $intdemandnum, intdemandval = intdemandval + $intdemandval"; 

					//$demandweekresult = mysqli_query($link, $demandweekquery) or die ("Error in query: $demandweekquery. ".mysqli_error($link));
