<?php
	...
	header('Content-Disposition: attachment; filename=customerpac2sales.csv');
	...
	while ($custrow = mysqli_fetch_row($custresult)) 
	{
		...
		$pac2query = "SELECT code FROM pac2";
		...
		while ($pac2row = mysqli_fetch_row($pac2result)) 
		{
			...
			for ($x = 0; $x <= 35 ; $x++)
			{
				...
				$query = "SELECT $salesfield FROM customerpac2sales WHERE account = '$account' AND pac2code = '$pac2code'";
				...
				// Write to the csv
				fputcsv($f, array($account, $codelevel, $pac2code, $yearmonth, $salesvalue));
			}
		}
	}
