<?php
	...
	header('Content-Disposition: attachment; filename=customerpac1sales.csv');
	...
	while ($custrow = mysqli_fetch_row($custresult)) 
	{
		...
		$pac1query = "SELECT code FROM pac1";
		$pac1result = mysqli_query($link, $pac1query) or logerror($pac1query,mysqli_error($link));
		while ($pac1row = mysqli_fetch_row($pac1result)) 
		{
			$pac1code = $pac1row[0];
			...
			for ($x = 0; $x <= 35 ; $x++)
			{
				...
				$query = "SELECT $salesfield FROM customerpac1sales WHERE account = '$account' AND pac1code = '$pac1code'";
				...
				// Write to the csv
				fputcsv($f, array($account, $codelevel, $pac1code, $yearmonth, $salesvalue));
			}
		}
	}
